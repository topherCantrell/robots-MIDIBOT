...
; ISR should count to 7 or 8 for current music delays. These should
; be adjusted to 0 (greatest delay is 64*3)
	...
	mov     SNGFLG,#0       ; Make sure Music is OFF
	ORL     IE,#1
	;mov    IE,#0x81        ; Only 120Hz interrupt
	
	lcall   MUSIC_INIT      ; Initialize the sound chip
	MOV     R0,#0xA0        ; Album at 0xA000
	MOV     R1,#0           ; '
	MOV     A,#1            ; Play song 1
	LCALL   PLAY_SONG       ; '
...
; Process music
	...
	mov     TICKCNT,#4      ; Restore tick counter
